...
/**
 * IndustryFilterBar v3
 * ─────────────────────────────────────────────────────────────────────────────
 * Unified search + two-level industry filter bar.
 *
 * Layout:
 *   Row 1 — Search | Level-1 tabs | [Right buttons]   ← always one row
 *   Row 2 — Level-2 sub-sectors                         ← only when L1 active
 */
...
interface IndustryFilterBarProps {
  ...
  extra?: React.ReactNode;   // <-- right-side buttons (Free/Premium etc.)
...
export default function IndustryFilterBar({
  ...
  return (
    <div className="space-y-2">
      {/* ── Row 1: Search | Level-1 tabs | [extra] — always one row ─────────── */}
      <div className="flex items-center gap-2">
        ...
        {showSearch && (
          <div className="relative w-52 shrink-0">
            ...
            <input
              ...
              className="w-full bg-gray-900/80 border border-gray-700/50 rounded-xl pl-10 pr-8 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/70 focus:bg-gray-900 transition-all duration-200"
            />
        ...
        {/* Level-1 tabs */}
        <div className="flex items-center gap-1 overflow-x-auto pb-0.5 scrollbar-hide flex-1 min-h-[40px]">
          {isUrlMode && allHref ? (
            <a
              href={allHref}
              className={`shrink-0 px-3.5 py-2 rounded-xl text-xs font-semibold transition-all duration-200 whitespace-nowrap ${
                industry === ''
        ...
        {/* Right-side extra buttons — always on this row */}
        {extra && <div className="flex items-center gap-1.5 shrink-0">{extra}</div>}
      </div>

      {/* ── Row 2: Level-2 sub-sectors — only when a L1 industry is active ─── */}
