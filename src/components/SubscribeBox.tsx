...
export default function SubscribeBox({
  ...
  const { user: session, loading: checking, refresh } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (loginData.exists) {
        // Auto-logged in — refresh auth context so session updates
        await refresh();
  ...
  // ── Auto-logged-in success (before session check — refresh may not have resolved yet) ──
